...
bool Frame::processMe()
{
	...
	if( message.get< jsonxx::String >( dvlKeys::typeOfReport ) == dvlReportTypes::velocity )
	{
		...
		auto figureOfMerit = static_cast< double >( message.get< jsonxx::Number >( dvlKeys::velocity::figureOfMerit ) );
		auto altitude      = static_cast< double >( message.get< jsonxx::Number >( dvlKeys::velocity::altitude ) );
		auto velocityValid = static_cast< bool >( message.get< jsonxx::Boolean >( dvlKeys::velocity::velocityValid ) );

		DVLStatus status
		    = static_cast< DVLStatus >( static_cast< int >( message.get< jsonxx::Number >( dvlKeys::status ) ) );

		this->content
		    = ProcessedVelocityFrame( timeFromLastReport, vx, vy, vz, figureOfMerit, altitude, velocityValid, status );
	...
	{
		auto timeStamp = static_cast< double >( message.get< jsonxx::Number >( dvlKeys::deadReckoning::timeStamp ) );
