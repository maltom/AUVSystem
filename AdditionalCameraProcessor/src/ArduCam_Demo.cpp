...
//
#include "external/arducam/ArduCamLib.h"
#include <unistd.h>
#include <termios.h>
...
void configBoard( ArduCamHandle& cameraHandle, Config config )
{
	...
	ArduCam_setboardConfig(
	    cameraHandle, config.params[ 0 ], config.params[ 1 ], config.params[ 2 ], config.params[ 3 ], u8Buf );
}
	...
	while( _running )
	{
		if( ArduCam_availableImage( handle ) > 0 )
		{
			...
			if( rtn_val == USB_CAMERA_NO_ERROR )
			{
				...
				// Mat showImage;
				// cv::resize(rawImage, showImage, cv::Size(640, (int)(rawImage.rows * scale)), (0, 0), (0, 0),
				// cv::INTER_LINEAR); cv::imshow("ArduCam", showImage);
